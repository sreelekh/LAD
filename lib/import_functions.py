...
def sensor_ts(sensor_id, df, temp_res):
    temp_rez_order = ['Year', 'Month', 'Week', 'Day', 'Hour', '15_Min', 'Min']
    for n, t in enumerate(temp_rez_order):
        if t == temp_res:
            cols = temp_rez_order[:n]
            return df.groupby(by=cols).sum().reset_index()[sensor_id]
